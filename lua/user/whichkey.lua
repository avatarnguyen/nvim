...
local mappings = {
  ...
  ["F"] = {
    "<cmd>lua require('telescope.builtin').find_files({file_ignore_patterns = { 'linux/', 'windows/', 'web/', 'doc/'}})<cr>",
    "Find files",
  },
  -- ["f"] = {
  ...
  s = {
    ...
    --   "Search Recent files" },
    d = { "<cmd>lua require('telescope.builtin').lsp_document_symbols()<cr>",
      "Document Symbols" },
    k = { "<cmd>lua require('telescope').extensions.recent_files.pick()<cr>",
      "Recent Files" },
    l = {
      "<cmd>lua require('telescope.builtin').live_grep({ file_ignore_patterns = {'*.gr.dart', 'linux/', 'macos/', 'windows/', 'ios/', 'android/', 'assets/', 'fonts/', 'packages/', 'doc/', 'l10n/'} })<cr>",
      "Find Text" },
  ...
  g = {
    ...
    o = { "<cmd>lua require('user.telescope').delta_git_status()<cr>", "Git Status" },
    ...
    e = { "<cmd>DiffviewOpen<cr>", "Open DiffView" },
  ...
  d = {
    name = "Flutter",
    n = { "<cmd>!tmux send-keys -t flutter 'fr' Enter<CR>", "Tmux: Run" },
    r = { "<cmd>!tmux send-keys -t flutter 'ffr' Enter<CR>", "Tmux: Run Flavor" },
    j = { "<cmd>!tmux send-keys -t flutter 'frd' Enter<CR>", "Tmux: Run Samsung Devices" },
    f = { "<cmd>!tmux send-keys -t flutter 'ffrd' Enter<CR>", "Tmux: Run Flavor Samsung Devices" },
    s = { "<cmd>!tmux send-keys -t flutter 'R'<CR>", "Tmux: Restart" },
    d = { "<cmd>!tmux send-keys -t flutter 'r'<CR>", "Tmux: Refresh" },
    v = { "<cmd>!tmux send-keys -t flutter 'v'<CR>", "Tmux: Start Dev Tool" },
    q = { "<cmd>!tmux send-keys -t flutter 'q'<CR>", "Tmux: Quit Flutter" },
    m = { ":FlutterRename<CR>", "Flutter Rename" },
    ...
    a = { "<cmd>lua require('user.lsp.handlers').code_action_fix_all()<cr><cmd>w!<cr>", "Fix All" },
